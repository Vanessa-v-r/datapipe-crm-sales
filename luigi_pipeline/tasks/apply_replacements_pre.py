# tasks/apply_replacements_pre.py
...
class ApplyReplacementsPreNormalization(BaseTask):
    force_once = luigi.BoolParameter(
        default=False,
        description="Force ApplyReplacementsPreNormalization to run once"
    )

    def requires(self):
        # Depends on ExtractSelectedProperties
        return ExtractSelectedProperties(force_once=False)

    def output(self):
        outputs = []
        for input_target in self.input():
            # Output path for pre-normalization replacements
            output_filename = os.path.basename(input_target.path).replace('processed', 'replaced_pre')
    ...
    @log_function
    def run_task(self):
        self.logger.info(event="start_task", message="Starting ApplyReplacementsPreNormalization task.")
        try:
            ...
            self.logger.info(event="end_task", message="Completed ApplyReplacementsPreNormalization task.")
        except Exception as e:
            self.logger.error(
                event="task_exception",
                message=f"Error in ApplyReplacementsPreNormalization task: {str(e)}",
                exception=str(e)
            )
            raise
